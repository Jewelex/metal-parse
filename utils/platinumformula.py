...
def calculate_platinum_rate(
    kitco_rate=1906,          # Kitco platinum USD/oz
    premium=0.02,             # Premium %
    ...
    F14 = (custom_duty * 1.01) * F13
    F15 = F13 * aidc
    total_usd = F13 + F14 + F15
    F17 = total_usd * forex_rate           # rate per gram 999 purity in INR
    F18 = F17 * jewellery_pct              # rate per gram ~950 purity
    F20 = F18 + spl_alloy_charges
    F21 = F20 * gst_pct
    F23 = F20 + F21
    F25 = F23
    F30 = F25 * making_rate
    F31 = F25 + F30                        # final per gram at ~950 level

    # 999 purity: scale up from 950 base
    rate_999_per_gram = (F31 / 950) * 999
    rate_999_10gm = math.ceil(rate_999_per_gram * 10 / 5) * 5

    # 950 purity: use as-is, round to nearest 5
    rate_950_per_gram = F31
    rate_950_10gm = round(rate_950_per_gram * 10 / 5) * 5

    return {
        "rate_for_10gm_999": rate_999_10gm,
        "rate_for_10gm_950": rate_950_10gm,
    }
